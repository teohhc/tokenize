/* jshint browser: true, esversion: 5, asi: true */
/*globals Vue, uibuilder */
// @ts-nocheck
/*
  Copyright (c) 2021 Julian Knight (Totally Information)

  Licensed under the Apache License, Version 2.0 (the "License");
  you may not use this file except in compliance with the License.
  You may obtain a copy of the License at

  http://www.apache.org/licenses/LICENSE-2.0

  Unless required by applicable law or agreed to in writing, software
  distributed under the License is distributed on an "AS IS" BASIS,
  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
  See the License for the specific language governing permissions and
  limitations under the License.
*/
'use strict'

/** @see https://totallyinformation.github.io/node-red-contrib-uibuilder/#/front-end-library */

// eslint-disable-next-line no-unused-vars
const app = new Vue({
    el: '#app',

    data() { return {

        startMsg    : 'Vue has started, waiting for messages',
		module      : 'document',
        feVersion   : '',
        counterBtn  : 0,
		language    : [{text:'English', value:'English'},
				{text:'Malay', value:'Malay'},
				{text:'Others', value:'Others'}],
		lang        : [],
		docList     : null,
		docTable    : null,
		docData     : [],
		docCol      : [],
		sheetTable  : null,
		sheetData   : [],
		sheetCol    : [],
		colList     : [],
		col         : null,
		dicList     : [],
		dic         : [],
		doc         : null,
		filename    : null,
		showXlsx    : false,
		showDocx    : false,
		showSheet   : false,
		sheet       : null,
		sheetList   : [],
		vis_rel     : null,
		vis_er      : null,
        inputText   : null,
        inputChkBox : false,
        socketConnectedState : false,
        serverTimeOffset     : '[unknown]',
        imgProps             : { width: 75, height: 75 },

        msgRecvd    : '[Nothing]',
        msgsReceived: 0,
        msgCtrl     : '[Nothing]',
        msgsControl : 0,

        msgSent     : '[Nothing]',
        msgsSent    : 0,
        msgCtrlSent : '[Nothing]',
        msgsCtrlSent: 0,

        isLoggedOn  : false,
        userId      : null,
        userPw      : null,
        inputId     : '',
		menu        : []

    }}, // --- End of data --- //

    computed: {

        hLastRcvd: function() {
            var msgRecvd = this.msgRecvd
            if (typeof msgRecvd === 'string') return 'Last Message Received = ' + msgRecvd
            else return 'Last Message Received = ' + this.syntaxHighlight(msgRecvd)
        },
        hLastSent: function() {
            var msgSent = this.msgSent
            if (typeof msgSent === 'string') return 'Last Message Sent = ' + msgSent
            else return 'Last Message Sent = ' + this.syntaxHighlight(msgSent)
        },
        hLastCtrlRcvd: function() {
            var msgCtrl = this.msgCtrl
            if (typeof msgCtrl === 'string') return 'Last Control Message Received = ' + msgCtrl
            else return 'Last Control Message Received = ' + this.syntaxHighlight(msgCtrl)
        },
        hLastCtrlSent: function() {
            var msgCtrlSent = this.msgCtrlSent
            if (typeof msgCtrlSent === 'string') return 'Last Control Message Sent = ' + msgCtrlSent
            //else return 'Last Message Sent = ' + this.callMethod('syntaxHighlight', [msgCtrlSent])
            else return 'Last Control Message Sent = ' + this.syntaxHighlight(msgCtrlSent)
        },

		doVisRel: function() {
			return this.vis_rel;
		},

		doVisEr: function() {
			return this.vis_er;
		},

    }, // --- End of computed --- //

    methods: {

        makeToast: function(mesg, append = true) {
            app.toastCount++
            app.$bvToast.toast(mesg, {
                title: 'Notification',
                toaster: 'b-toaster-top-center',
                variant: 'success',
                autoHideDelay: 5000,
                appendToast: append
            });
        },


        // Called from the increment button - sends a msg to Node-RED
        increment: function(event) {
            // console.log('Button Pressed. Event Data: ', event)

            // Increment the count by one
            this.counterBtn = this.counterBtn + 1
            var topic = this.msgRecvd.topic || 'uibuilder/vue'
            uibuilder.send( {
                'topic': topic,
                'module': this.module,
                'payload': {
                    'type': 'counterBtn',
                    'btnCount': this.counterBtn,
                    'message': this.inputText,
                    'inputChkBox': this.inputChkBox
                }
            } )

        }, // --- End of increment --- //

        // REALLY Simple method to return DOM events back to Node-RED. See the 2nd b-button on the default html
        // doEvent: uibuilder.eventSend,
        doReadDoc: function(payload) {
            // console.log('Button Pressed. Event Data: ', event)
            // console.log(payload)
            var topic = payload.instruction || 'uibuilder/vue'
			uibuilder.send( {
                'topic': topic,
                'module': this.module,
				'payload': payload
			} )
		},

        doReadDic: function(payload) {
            // console.log('Button Pressed. Event Data: ', event)
            // console.log(payload)
			var payload = {
				'instruction': 'readDic',
				'language': this.lang
			}
			this.doReadDoc(payload);
		},


        doRead: function(event) {
			var ext = this.doc.split(".")
			var max = ext.length
			switch (ext[max-1]) {
				case 'docx':
					var payload = {
						'instruction': 'readDocx',
						'filename': this.doc,
						'language': this.lang,
						'optional': this.dic
					}
					this.doReadDoc(payload);
					break;
				case 'xlsx':
					var payload = {
						'instruction': 'readXlsx',
						'filename': this.doc,
						'sheet': this.sheet,
						'column': parseInt(this.col),
						'language': this.lang,
						'optional': this.dic
					}
					this.doReadDoc(payload);
					break;
			}
		},

		switchDoc: function(event) {
			var ext = event.split(".")
			var max = ext.length
			this.docTable.setColumns([])
			this.docTable.setData([])
			if (this.sheetTable !== null) {
				delete this.sheetTable;
			}
			this.showSheet = false;
			switch (ext[max-1]) {
				case 'docx':
					this.showDocx = true;
					this.showXlsx = false;
					this.showSheet = false;
					break;
				case 'xlsx':
					this.showDocx = false;
					this.showXlsx = true;
					this.showSheet = false;
					var payload = {
						'instruction': 'readFile',
						'filename': this.doc
					}
					this.doReadDoc(payload);
					break;
			}
		},

		switchSheet: function(event) {
            var topic = 'readSheet' || 'uibuilder/vue'
			this.showSheet = true;
			uibuilder.send( {
                'topic': topic,
                'module': this.module,
				'payload': {
					'instruction': 'readSheet',
					'filename': this.doc,
					'sheet': this.sheet
				}
			} )
		},

		switchCol: function(event) {

		},

		doSubmit: function(event) {
			var tmpext = this.doc.split(".")
			var max = tmpext.length
			var ext = tmpext[max-1]
			var data = this.docTable.getData();

			switch (ext) {
				case 'docx':
					var payload = {
						'instruction': 'submitToken',
						'ext':ext,
						'filename': this.doc,
						//'language': Array.isArray(this.lang)?this.lang:Array(this.lang),
						'language': this.lang,
						'data': data
					}
					this.doReadDoc(payload);
					break;
				case 'xlsx':
					var payload = {
						'instruction': 'submitToken',
						'ext':ext,
						'filename': this.doc,
						'sheet': this.sheet,
						'column': parseInt(this.col),
						'language': this.lang,
						'data': data
					}
					this.doReadDoc(payload);
					break;
			}
		},

		doServerReq: function(instruction) {
			var tmpext = this.doc.split(".")
			var max = tmpext.length
			var ext = tmpext[max-1]
			var payload = null;
			switch (ext) {
				case 'docx':
					payload = {
						'instruction': instruction,
						'ext':ext,
						'filename': this.doc,
						//'language': Array.isArray(this.lang)?this.lang:Array(this.lang),
						'language': this.lang,
					}
					break;
				case 'xlsx':
					payload = {
						'instruction': instruction,
						'ext':ext,
						'filename': this.doc,
						'sheet': this.sheet,
						'column': parseInt(this.col),
						'language': this.lang,
					}
					break;
			}
			uibuilder.send( {
                'topic': payload.instruction,
                'module': this.module,
				'payload': payload
			});
		},

		doVisualize: function(event) {
			this.doServerReq('visualize');
		},

		dl_er: function() {
			this.doServerReq('downloadER');
		},

		dl_el: function() {
			this.doServerReq('downloadEL');
		},

        // return formatted HTML version of JSON object
        syntaxHighlight: function(json) {
            json = JSON.stringify(json, undefined, 4)
            json = json.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
            json = json.replace(/("(\\u[a-zA-Z0-9]{4}|\\[^u]|[^\\"])*"(\s*:)?|\b(true|false|null)\b|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?)/g, function (match) {
                var cls = 'number'
                if (/^"/.test(match)) {
                    if (/:$/.test(match)) {
                        cls = 'key'
                    } else {
                        cls = 'string'
                    }
                } else if (/true|false/.test(match)) {
                    cls = 'boolean'
                } else if (/null/.test(match)) {
                    cls = 'null'
                }
                return '<span class="' + cls + '">' + match + '</span>'
            })
            return json
        }, // --- End of syntaxHighlight --- //

    }, // --- End of methods --- //

    // Available hooks: beforeCreate,created,beforeMount,mounted,beforeUpdate,updated,beforeDestroy,destroyed, activated,deactivated, errorCaptured

    /** Called after the Vue app has been created. A good place to put startup code */
    created: function() {

        // Example of retrieving data from uibuilder
        this.feVersion = uibuilder.get('version')

        /** **REQUIRED** Start uibuilder comms with Node-RED @since v2.0.0-dev3
         * Pass the namespace and ioPath variables if hosting page is not in the instance root folder
         * e.g. If you get continual `uibuilderfe:ioSetup: SOCKET CONNECT ERROR` error messages.
         * e.g. uibuilder.start('/uib', '/uibuilder/vendor/socket.io') // change to use your paths/names
         * @param {Object=|string=} namespace Optional. Object containing ref to vueApp, Object containing settings, or String IO Namespace override. changes self.ioNamespace from the default.
         * @param {string=} ioPath Optional. changes self.ioPath from the default
         * @param {Object=} vueApp Optional. Reference to the VueJS instance. Used for Vue extensions.
         */
        uibuilder.start(this) // Single param passing vue app to allow Vue extensions to be used.

		//console.log(JSON.stringify(this.docData));


        //console.log(this)
		uibuilder.send( {
			'module': this.module,
			'payload': {
				'instruction': 'getFileList',
			}
		} );

    }, // --- End of created hook --- //

    /** Called once all Vue component instances have been loaded and the virtual DOM built */
    mounted: function(){

		this.docTable = new Tabulator("#doc-table", {
			minHeight: "300px", 
			maxHeight: "500px", 
			pagination: "local",
			paginationSize: 10,
			autoResize: true,
			resizableColumns: true,
			virtualDomHoz:true,
			data: this.docData, //assign data to table
			layout: "fitColumns", //fit columns to width of table (optional)
			columns: this.docCol
		});


        //console.debug('[indexjs:Vue.mounted] app mounted - setting up uibuilder watchers')

        var app = this  // Reference to `this` in case we need it for more complex functions

        // If msg changes - msg is updated when a standard msg is received from Node-RED over Socket.IO
        uibuilder.onChange('msg', function(msg){
            // console.info('[indexjs:uibuilder.onChange] msg received from Node-RED server:', msg)
			// console.info(JSON.stringify(msg))
            app.msgRecvd = msg
            app.msgsReceived = uibuilder.get('msgsReceived')
			switch (msg.topic) {
				case "listFiles":
					app.docList = msg.payload.docList
					break;
				case "docTable":
					app.docData = msg.payload.data
					app.docCol = msg.payload.config.col
					app.docTable.setColumns(app.docCol)
					app.docTable.setData(app.docData)
					break;
				case "readFile":
					app.sheetList = msg.payload.SheetNames
					app.showXlsx = true
					break;
				case "readSheet":
					app.showSheet = true
					app.sheetData = msg.payload.data
					app.sheetCol = msg.payload.config.col
					// console.log("docSheet");
					// console.log(app.sheetCol);
					// console.log(app.sheetData);
					if (this.sheetTable !== null) {
						delete this.sheetTable;
					}
					this.sheetTable = new Tabulator("#sheet-table", {
						minHeight: "300px", 
						maxHeight: "500px", 
						pagination: "local",
						paginationSize: 10,
						autoResize: true,
						resizableColumns: true,
						virtualDomHoz:true,
						data: app.sheetData, //assign data to table
						layout: "fitColumns", //fit columns to width of table (optional)
						columns: app.sheetCol
					});
					var tempColList = [];
					for (var idx in app.sheetCol) {
						tempColList.push(idx);
					}
					app.colList = tempColList;
					// console.log("done");
					break;
				case "notification":
					//console.log(msg.payload);
					//alert(msg.payload.data);
					app.makeToast(msg.payload.data, true);
					break;
				case "svg_rel":
					app.vis_rel = msg.payload.data;
					break;
				case "svg_er":
					app.vis_er = msg.payload.data;
					break;
				case "menu":
					app.menu = msg.payload.menu;
					break;
				case "dicList":
					app.dicList = msg.payload.data;
					break;
				case "spreadsheet":
					window.open(msg.payload.url, "_blank");
					break;
				case "svg":
					window.open(msg.payload.url, "_blank");
					break;
				/*
				case "spreadsheet":
					var newSheetList={};
					for (var idx in msg.payload.data.rows) {
						var rec = msg.payload.data.rows[idx];
					}
					this.docTable = new Tabulator("#doc-table", {
						minHeight: "300px", 
						maxHeight: "500px", 
						pagination: "local",
						paginationSize: 10,
						autoResize: true,
						resizableColumns: true,
						virtualDomHoz:true,
						data: this.docData, //assign data to table
						layout: "fitColumns", //fit columns to width of table (optional)
						columns: this.docCol
					});
					break;
				*/

			}
        })

        //#region ---- Debug info, can be removed for live use ---- //

        /** You can use the following to help trace how messages flow back and forth.
         * You can then amend this processing to suite your requirements.
         */

        // If we receive a control message from Node-RED, we can get the new data here - we pass it to a Vue variable
        uibuilder.onChange('ctrlMsg', function(msg){
            //console.info('[indexjs:uibuilder.onChange:ctrlMsg] CONTROL msg received from Node-RED server:', msg)
            app.msgCtrl = msg
            app.msgsControl = uibuilder.get('msgsCtrl')
        })

        /** You probably only need these to help you understand the order of processing
         * If a message is sent back to Node-RED, we can grab a copy here if we want to
         */
        uibuilder.onChange('sentMsg', function(msg){
            //console.info('[indexjs:uibuilder.onChange:sentMsg] msg sent to Node-RED server:', msg)
            app.msgSent = msg
            app.msgsSent = uibuilder.get('msgsSent')
        })

        /** If we send a control message to Node-RED, we can get a copy of it here */
        uibuilder.onChange('sentCtrlMsg', function(msg){
            //console.info('[indexjs:uibuilder.onChange:sentCtrlMsg] Control message sent to Node-RED server:', msg)
            app.msgCtrlSent = msg
            app.msgsCtrlSent = uibuilder.get('msgsSentCtrl')
        })

        /** If Socket.IO connects/disconnects, we get true/false here */
        uibuilder.onChange('ioConnected', function(connected){
            // console.info('[indexjs:uibuilder.onChange:ioConnected] Socket.IO Connection Status Changed to:', connected)
            app.socketConnectedState = connected
			
        })
        /** If Server Time Offset changes */
        uibuilder.onChange('serverTimeOffset', function(serverTimeOffset){
            //console.info('[indexjs:uibuilder.onChange:serverTimeOffset] Offset of time between the browser and the server has changed to:', serverTimeOffset)
            app.serverTimeOffset = serverTimeOffset
        })

        /** If user is logged on/off */
        uibuilder.onChange('isAuthorised', function(isAuthorised){
            //console.info('[indexjs:uibuilder.onChange:isAuthorised] isAuthorised changed. User logged on?:', isAuthorised)
            //console.log('authData: ', uibuilder.get('authData'))
            //console.log('authTokenExpiry: ', uibuilder.get('authTokenExpiry'))
            app.isLoggedOn = isAuthorised
        })

        //#endregion ---- Debug info, can be removed for live use ---- //

    }, // --- End of mounted hook --- //

}) // --- End of app1 --- //

// EOF
